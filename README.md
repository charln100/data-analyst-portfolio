# Data Analyst Portfolio

## About Me
I am on a learning journey to become a data analyst. This portfolio documents my projects and progress as I build real-world skills.

## Projects

### 1. Superstore Sales Analysis
**Tools used:** Google Sheets  
**Dataset:** Superstore Sales (9,994 orders)  

**What I analyzed:**
- Total revenue and average order value
- Sales performance by region
- Sales performance by product category

**Key Findings:**
- Total revenue: $2,297,200.86
- West region leads in sales at $725,457 — 85% higher than the South region ($391,721)
- Technology is the top product category at $836,154, with all three categories within a relatively tight range
- Average order value: $229.86, with a maximum single order of $22,638.48
- Identified data quality issues including ship dates preceding order dates, indicating data entry errors in the source dataset

**Skills demonstrated:** Data cleaning(including date formatting issues), pivot tables, charts, business insight writing

**Analysis Summary:**
The Superstore dataset contains 9,994 orders totaling $2.3M in revenue with an average order value of $229.86. Geographically, the West leads with $725K in sales while the South significantly underperforms at $392K, raising the question of whether this gap is driven by population size, product availability, or sales team performance.

At the product level, Technology leads in sales revenue despite having fewer orders than Office Supplies, indicating Technology generates higher value per order while Office Supplies drives volume. Furniture comes in second in revenue.

Operationally, Standard Class is the dominant shipping method, suggesting customers prioritize cost savings over delivery speed. California, New York, and Texas, the three largest states by population, lead in total sales revenue, confirming the strong influence of population size on demand.

---

## Skills In Progress
- Google Sheets ✅
- SQL (learning)
- Python (coming soon)
- Tableau (coming soon)
